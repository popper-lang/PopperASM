...
impl Parser {
    ...
    fn parse_register(&mut self) -> Result<Register, Error> {
        ...
        let register = match register.token_kind {
            ...
            _ => return Err(Error::new(format!("Expected ident, found {:?}", register.token_kind), register.span))
        };
        let register = match register.lexeme.as_str() {
            ...
            "r15" => Register::R15,
            "rbp" => Register::Rbp,
            _ => return Err(Error::new(format!("Expected register, found {:?}", register.lexeme), register.span))
        };
    ...
    fn parse_memory_fetching(&mut self) -> Result<MemoryFetching, Error> {
        match self.peek().token_kind {
            ...
            e => Err(Error::new(format!("Expected ident or hashtag, found {:?}", e), self.peek().span))
        }
    }
    ...
    fn parse_int(&mut self) -> Result<Expr, Error> {
        ...
        let int = match int.token_kind {
            TokenKind::Int => int,
            _ => return Err(Error::new(format!("Expected int, found {:?}", int.token_kind), int.span))
        };
        let int = match int.lexeme.parse::<i32>() {
            Ok(int) => int,
            Err(_) => return Err(Error::new(format!("Expected int, found {:?}", int.lexeme), int.span))
        };
        Ok(Expr::Int(int))
    }
    ...
    fn parse_expr(&mut self) -> Result<Expr, Error> {
        ...
        let expr = match expr.token_kind {
            ...
            _ => return Err(Error::new(format!("Expected int, ident or hashtag, found {:?}", expr.token_kind), expr.span))
        };
        Ok(expr)
    }
    ...
    fn parse_addr(&mut self) -> Result<usize, Error> {
        self.advance();
        let n = self.expect(TokenKind::Int)?
            .lexeme
            .parse::<usize>()
            .unwrap();
        Ok(n)
    ...
    fn expect(&mut self, token_kind: TokenKind) -> Result<Token, Error> {
        ...
        if token.token_kind == token_kind {
            ...
            Err(Error::new(format!("Expected {:?}, found {:?} `{}`", token_kind, token.token_kind, token.lexeme), token.span))
        }
    }
